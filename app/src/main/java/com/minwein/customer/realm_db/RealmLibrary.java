package com.minwein.customer.realm_db;
...
public class RealmLibrary {
    ...
    public void insertItem(final Context context, final String itemId,
                           ...
                           final String General_request, String itemQuantity, String vendorName) {
            ...
            if (cartRealmModel != null) {
                ...
                if (!updateIngredients) {
                    ...
                    for (int count = 0; count < ingredientsList.size(); count++) {
                        ...
                        ingredientsRealmModel.setIngredientsQuantity(ingredient.getIngredientsKey());
    ...
    public void deleteItem(final Integer id) {
        Realm realm = Realm.getDefaultInstance();
        realm.beginTransaction();
        CartRealmModel item = realm.where(CartRealmModel.class).equalTo("id", id).findFirst();
        if(item!=null){
    ...
    public void deleteVendorItem(String vendor_key) {
        Realm realm = Realm.getDefaultInstance();
        realm.beginTransaction();
        RealmResults<CartRealmModel> vendorList = realm.where(CartRealmModel.class).equalTo("vendor_key", vendor_key).findAll();
        vendorList.deleteAllFromRealm();
        realm.commitTransaction();
    }
}